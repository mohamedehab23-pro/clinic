...
const schema=zod.object({
  name:zod.string('name is required').nonempty().min(3,'name must be at least 3')
})
        ...
        <div className='flex justify-center items-center'>
          ...
          <div className='  p-6 dark:text-white dark:bg-gray-800 bg-white rounded-xl min-w-md border border-gray-200'>
            <form onSubmit={handleSubmit(sendData)} className='flex text-end flex-col gap-4'>
              ...
              <Input errorMessage={errors.name?.message} isInvalid={errors.name} variant='bordered' {...register('name')} id='name' />
              <label htmlFor='mobile'>رقم الموبايل</label>
              <Input errorMessage={errors.phone?.message} isInvalid={errors.phone} variant='bordered' {...register('phone')} id='mobile' />
              <label htmlFor='service'>نوع الخدمة</label>
              <select isInvalid errorMessage  className='p-2 rounded-xl shadow border border-gray-300' {...register('service')} id="jobs">
                <option value=""></option>
