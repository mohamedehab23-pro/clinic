...
window.open(whatsappURL,'_blank')
  ...
  return (
    <>
      <section  id='booking' className='min-h-[50vh] w-full py-12 '>
...
<Header text={'احجز موعدك'} />
        <div className='flex justify-center  items-center'>
          ...
          <div className='  p-6 dark:text-white dark:bg-gray-800 bg-white rounded-xl lg:min-w-md  w-sm border border-gray-200'>
            ...
            {errors.service&& touchedFields.service&& (<p className='text-red-700 text-xs '>{errors.service?.message}</p>)}
              <label  htmlFor='date' >الموعد المفضل</label>
              <input    type="date" className={`p-2 rounded-xl shadow border text-sm border-gray-300 
                ${errors.time ?'border-red-[#562243]':'border-gray-300' }`} {...register('time')}/>
             {errors.time&& touchedFields.time&& <p className='text-red-700 text-xs'>{errors.time?.message}</p>}
