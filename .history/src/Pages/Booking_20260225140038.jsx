...
export default function Booking() {
 ...
 const [name, setName] = useState("");
  ...
  const [service, setService] = useState(bookingServiceOptions[0]?.value ?? "");
...
function formHandle(e){
 e.preventDefault();
    ...
    const text = [
      ...
      `الاسم: ${name || "-"}`,
      `رقم الموبايل: ${phone || "-"}`,
      `نوع الخدمة: ${bookingServiceOptions.find((o) => o.value === service)?.label ?? service}`,
...
<form onClick={()=>{ha}} className='flex text-end flex-col gap-4'>
